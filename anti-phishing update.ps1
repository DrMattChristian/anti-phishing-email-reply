...
foreach ($i in $data)
	{
		new-mailcontact -name $i.email -externalemailaddress $i.email -organizationalunit $ou
		add-distributionGroupMember -identity eduphishing -member $i.email
	}
get-mailcontact -organizationalunit "edu phishing" | set-mailcontact -customattribute1 $ou -HiddenFromAddressListsEnabled:$true